...
import edlib
from utils import read, check_dir_exist1, check_dir_exist2, DrawGeneFig1, DrawGeneFig2, DNA_complement2, Compute_N50
...
hairpin1 = args.hairpin1_seq
hairpin2 = args.hairpin2_seq

windows_len = min(len(hairpin1),len(hairpin2))
step_len = args.step_len
gap_len = args.poly_len

LD_thre = np.ceil(windows_len*args.err_threshold)
adjust_dis = args.adjusted_len
hairpin_len = len(args.hairpin1_seq+args.hairpin2_seq)+args.poly_len
...
raw_subreads_1_8_len = {}          #记录subread1-8各自的subread长度   表格第四部分
...
for read_ele in lines_gen:
    ...
    for i in range((np.ceil(len(read_ele[1]) - windows_len)/step_len).astype("int")):
        ld1 = distance(read_ele[1][i*step_len:i*step_len+len(hairpin1)], hairpin1)
        ld2 = distance(read_ele[1][i*step_len:i*step_len+len(hairpin2)], hairpin2)
        if ld1 <=LD_thre or  ld2 <= LD_thre :
            if ld1 <= ld2:
                locs.append(i*step_len)
                LD_list.append((ld1,ld2))
            else :
                locs.append(i*step_len-gap_len-len(hairpin1))
                LD_list.append((ld1,ld2))
    ...
    if locs == [] :   #没有hairpin被识别到
        ...
        for j in range(len(locs)):
            if LD_list[j].index(min(list(LD_list[j]))) == 0 :
                if locs[j] <= adjusted_locs[-1] + adjust_dis:
                    continue
                else:
                    adjusted_locs.append(locs[j])
            elif LD_list[j].index(min(list(LD_list[j]))) == 1:
                if locs[j] - (windows_len + gap_len) <= adjusted_locs[-1] + adjust_dis:   #block1 42 gap 10
                    continue
                else:
                    adjusted_locs.append(locs[j])
    ...
    if adjusted_locs != [] :
        ...
        for subread_ele in range(len(subreads_list_raw)):
            if subread_ele+1 not in raw_subreads_1_8_len:
                raw_subreads_1_8_len[subread_ele+1] = []
                raw_subreads_1_8_len[subread_ele+1].append(len(subreads_list_raw[subread_ele]))
            else:
                raw_subreads_1_8_len[subread_ele+1].append(len(subreads_list_raw[subread_ele]))
            ...
            if len(subreads_list_tmp[0]) < 0.5*(len(subreads_list_tmp[1])):  #第一个的hairpin长度<第二个hairpin的50%
                ...
                if len(subread_filtered1_list) >=2:
                    ...
                    for ff in final_subread_idx:
                        try:
                            res_Identity = test_edlib(subreads_list_tmp_1[0], subreads_list_tmp_1[ff])
                        except:
                            res_Identity = 0
                    ...
                    if filter_second_subread_num >=3:
                        ...
                        if filter_second_subread_num >=3:
                            ...
                            f.close()
                            if draw_count <= 200:
                                DrawGeneFig1(adjusted_locs,read_ele[0][1:],len(read_ele[1]),hairpin1,hairpin2,gap_len,filter_first_hairpin_num,filter_second_subread_num,res_dir)
                                draw_count += 1
                    else:  #如果最终subread数目<3
                        if draw_count2 <= 200:
                            DrawGeneFig2(adjusted_locs,read_ele[0][1:],len(read_ele[1]),hairpin1,hairpin2,gap_len,filter_first_hairpin_num,res_dir)
                            draw_count2 += 1
...
for subread_idx, subread_len_tmp in  raw_subreads_1_8_len.items():
    if subread_idx <= 8:
...
for i in raw_subreads_1_8_len.values():
...
if 3 in subread_filter_2_read_dict:
...
if 3 in subread_filter_2_read_dict:
    ...
    print("Subread筛选后subread≥3的Read N50为: " +str(0))
if 4 in subread_filter_2_read_dict:
    print("Subread筛选后subread≥4的Read N50为: " +str(read_gt4_len_N50_2))
else:
    print("Subread筛选后subread≥4的Read N50为: " +str(0))
if 5 in subread_filter_2_read_dict:
