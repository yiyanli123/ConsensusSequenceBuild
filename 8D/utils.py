...
def DrawGeneFig1(start_list,readID,lens,hairpin1,hairpin2,gap_len,num_id,num_id2,res_dir):  #loc用这个adjusted_locs 
    features = []
    for start in start_list:
        features.append(GraphicFeature(start=start, end=start+len(hairpin1)+gap_len+len(hairpin2), strand=+1, color="#ffd700",label=str(start)))
...
def DrawGeneFig2(start_list, readID,lens, hairpin1, hairpin2, gap_len, num_id, res_dir):  #loc用这个adjusted_locs 
    features = []
    for start in start_list:
        features.append(GraphicFeature(start=start, end=start+len(hairpin1)+gap_len+len(hairpin2), strand=+1, color="#ffd700",label=str(start)))
    record = GraphicRecord(sequence_length=lens, features=features)
